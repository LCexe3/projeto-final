...
function vetMontarPerfilLocal(user = {}, veterinario = {}) {
    const nome = veterinario.nome || user?.user_metadata?.nome || user?.email || 'Veterinário CastraPrev';
    return {
        id: user?.id || veterinario.user_id || '',
        veterinarioId: veterinario.id || '',
        nome,
        apelido: user?.user_metadata?.apelido || vetPrimeiroNome(nome),
        email: veterinario.email || user?.email || '',
        ...
        criadoEm: veterinario.created_at || '',
        atualizadoEm: veterinario.updated_at || ''
    };
}

function vetSalvarSessaoSite(user = {}, veterinario = {}) {
    const perfil = vetMontarPerfilLocal(user, veterinario);
...
async function vetGarantirPerfilSite(user = {}, veterinario = {}) {
    ...
    const { data, error } = await supabaseClient
        ...
        .upsert({
            id: perfil.id,
            nome: perfil.nome,
            apelido: perfilAtual?.apelido || perfil.apelido,
            email: perfil.email,
            telefone: perfil.telefone || perfilAtual?.telefone || '',
            cep: perfilAtual?.cep || '',
            endereco: perfilAtual?.endereco || '',
            numero: perfilAtual?.numero || '',
            bairro: perfilAtual?.bairro || '',
            cidade: perfil.cidade || perfilAtual?.cidade || '',
            uf: perfil.uf || perfilAtual?.uf || '',
...
async function vetSessaoAtual() {
    ...
    await vetGarantirPerfilSite(user, veterinario);
    vetSalvarSessaoSite(user, veterinario);
